...
#'
#' The files to be combined must be transferred into the same folder, and this is where the new combined files will be placed.
...
#'
#' When combining the files, the function creates a seamless sequence of trajectory identifiers through the combined data, beginning from 1 for the first baseline (maximum likelihood) trajectory
#' of the first set.
#'
#' If for any reason there is a need to combine separate batches of multiple run results, then 
...
#'
#' \strong{Details relating to merging sensitivity analysis files:}
#'
...
#'
#' When combining sensitivity analysis data the first-named model.ident in the ident.list vector MUST correspond to a run of the e2e_run_sens() function with the argument coldstart=TRUE, and all others with coldstart=FALSE. This forces the
...
#'
#' \strong{Details relating to merging Monte Carlo analysis files:}
#'
#' e2e_run_mc() generates 11 different output files of accumulated outputs from the iterations (total 3.2 Mb per iteration) covering the range of model outputs. This merging function combines batches of each of these types of files. 
...
#' 
#' @param model Model object for the raw data to be combined generated by the e2e_read() function.
#' @param selection Text string from a list identifying source of data to be merged. Select from: "SENS", "MC", referring to sensitivity analysis or Monte Carlo analysis. Remember to include the phrase within "" quotes.
#' @param ident.list A vector of text variables corresponding to the "model.ident" identifiers for each of the files to merged (list must be length 2 or greater).
#' @param postprocess Logical. if TRUE then process the results through to final data; if FALSE just produce the combined raw results (default=TRUE). The reason for NOT processing would be if there are further run results stil to be combined with the set produced by this function.
#' @param csv.output Logical. If TRUE then enable writing of CSV output files (default=FALSE).
#'
#' @return csv files of merged data if csv.output=TRUE. If the argument postprocess=TRUE then also a dataframe of processed output (for sensitivity analysis) or a list object of dataframes (for Monte Carlo analysis).
#'
#' @seealso \code{\link{e2e_read}} , \code{\link{e2e_run_sens}} , \code{\link{e2e_run_mc}} , \code{\link{e2e_process_sens_mc}} , \code{\link{e2e_plot_sens_mc}} 
#'
#' @export
#'
#' @examples
...
e2e_merge_sens_mc <- function(model, selection="", ident.list, postprocess=TRUE, csv.output=FALSE) {
...
if(selection=="SENS"){
	...
	merged_data <- concatenate_raw_sensitivity_data_files(model=model, ident.list=ident.list, postprocess=postprocess)

} else if(selection=="MC"){
	merged_data <- concatenate_raw_mc_data_files(model=model, ident.list=ident.list,postprocess=postprocess,csv.output=csv.output)
