#
# generate_df_yield_curve_data.R
#
#' Run a set of models to generate fishery yield curve data for demersal fish.
#'
#' Perform a set of e2e_run() model runs along a sequence of values of demersal fish harvest ratio multiplier, saving the annual average whole-domain biomass, annual landings and annual discards of planktivorous and demersal fish and carn/scav feeding benthos from each run 
...
#'
#' The demersal fish yield curve can be generated for a given setting of planktivorous fish harvest ratio multiplier (default = 1.0). All other
...
#'
#' The demersal fish biomass and catches stored in the returned data object (and optionally a csv file) can subsequently be plotted using the function plot_df_yield_curve(). Users can easily plot any of the other 
...
#' @param DFHRvector A vector of ascending order unique multiplier values to be applied to the baseline demersal fish harvest ratio (default = c(0,0.5,1.0,1.5,2.0,2.5,3.0)).
#' @param PFHR Single value of the multiplier to be applied as the planktivorous fish harvest ratio for all runs (default=1.0).
#' @param csv.output Logical. If FALSE then disables writing of output to a csv file (default=TRUE).
#'
...
generate_df_yield_curve_data <- function(model,nyears=50,DFHRvector=c(0,0.5,1.0,1.5,2.0,2.5,3.0),PFHR=1,csv.output=TRUE ) {

# Check that the object DFHRvector is a vector of length >1
if(is.vector(DFHRvector)==FALSE || length(DFHRvector)<2 ){
	stop("Demersal fish harvest ratio multiplier needs to be a vector of length >1")
}

#Check that DFHRvector is an ascending order set of unique values
test<-1
for(jj in 2:(length(DFHRvector))){
if(DFHRvector[jj] <= DFHRvector[jj-1]) test<-0 
}
if(test==0 ){
	stop("Demersal fish harvest ratio multiplier must be a vector of ascending order unique values")
}
...
Nsteps<-(length(DFHRvector))

datastore<-data.frame(PlankFishHRmult=rep(PFHR,Nsteps),DemFishHRmult=DFHRvector,
...
message("Run ",kkk,"; df harvest ratio multiplier = ",DFHRvector[kkk])

model$data$fleet.model$HRscale_vector_multiplier[1]<-PFHR

model$data$fleet.model$HRscale_vector_multiplier[2]<-DFHRvector[kkk]
...
#-----------------------------------------------------------------
	...
	if(csv.local==TRUE){
	filename = csvname(resultsdir, "Yield_curve_data_DFHRmult", identifier)
