...
#' @param selection Text string from a list to select the method for optimization. Select from: "HR", "ECO", Remember to include the phrase within "" quotes (default = "HR"). "HR" selects a scheme for optimizing the activity multipler to known harvest ratios and uses just the fishing fleet model. "ECO" selects a scheme for optimizing to the observational data in the state of the ecosystem and so uses both the fishing fleet and ecology models.
#' @param n_iter Number of iterations of the model (per trajectory in the case of selection = "HR") (default=3000). Suggest a value of 500 if selection = "ECO".
#' @param start_temperature Initial value of the simulated annealing temperature parameter (default=1). Suggested a value of 0.5 if selection = "ECO". Values in the range 0.0005 - 5 are reasonable. Higher values increase the probability of rejecting parameter combinations producing an improvement in likelihood.
#' @param cooling Rate at which the simulated annealing temperature declines with iterations (default=0.985). Suggested a value of 0.975 if selection = "ECO".
...
#' @param n_traj Specific to selection="HR" : Number of repeats (trajectories) of the simulated annealing process (default=100).
#' @param deltaHi Specific to selection="HR" : Initial coefficient of variation for jiggling the activity multiplier values (default=0.2). This is set in the parameter file "optimize_fishing.csv" in the case where selection="ECO" and can be manually varied during a run.
#' @param attenuationstep Specific to selection="HR" : Number of itereations between down-steps of the jiggling factor applied to the multiplier values. The jiggling rate is attenuated by a factor of 2 every xx iterations (default=500).
#' @param deltaG Specific to selection="HR" : Coefficient of variation for jiggling the gear linkage values (default=0.25).
#' @param nyears Specific to selection="ECO" : Number of years to run the ecology model in each iteration (default=40).
#' @param quiet Logical. Specific to selection="ECO" : If TRUE then suppress informational messages at the start of each iteration (default=TRUE).
#'
#' @return A list object containing the data from each trajectory and the final accepted parameter values. Optionally (by default), csv files of the histories and the final accepted parameter values. The latter are returned to the model parameter folder in a format to be read back into the model.
#'
#' @seealso \code{\link{e2e_ls}} , \code{\link{e2e_read}} , \code{\link{e2e_plot_opt_diagnostics}} , \code{\link{e2e_optimize_eco}} , \code{\link{e2e_optimize_hr}}
#'
...
e2e_optimize_act <- function(model,selection="HR",n_iter=3000,start_temperature=0.5, cooling=0.985,
			     csv.output=FALSE, runtime.plot=TRUE,                          # up to here the arguments are generic for both selection options
                             n_traj=100, deltaHi=0.20, attenuationstep=500, deltaG=0.25,  # these are specific to the fitting to harvest ratios 
			     nyears=40, quiet=TRUE) {                                     # these are specific to the fitting to ecosystem state
...
if(selection=="HR"){
	fitted_data <- optimize_activity_mult_hr(model=model,
                                                n_traj=n_traj,
                                                n_iter=n_iter,
	...
	fitted_data <- optimize_activity_mult_eco(model=model,
                                                nyears=nyears,
                                                n_iter=n_iter,
                                                start_temperature=start_temperature,
                                                cooling=cooling,
